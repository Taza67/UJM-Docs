...
public class Client implements IConfig {
	...
	public void disconnect() {
		try {
			...
			e.printStackTrace();
			System.err.println(
					"- Client#disconnect() -> Échec de la déconnexion (Déconnexion interrompue par une erreur IO)");
		}
	}
	...
	public int waitInt() {
		try {
			...
			if (response == IMPOSSIBLE_CODE)
				// Avertissement
				System.err.println("- Client#WaitInt() -> Attention ! L'entier impossible " + IMPOSSIBLE_CODE
						+ " a été récupéré !");
			...
			e.printStackTrace();
			System.err.println(
					"- Client#waitInt() -> " + "Échec de la récupération de l'entier (Interruption par une erreur IO)");
		}
	...
	public String waitString() {
		try {
			...
			if (response == "")
				// Avertissement
				System.err.println(
						"- Client#WaitString() -> Attention !" + "Une chaîne de caractères vide a été récupérée");
	...
	public void sendInt(int value) {
		try {
			...
			e.printStackTrace();
			System.err.println(
					"- Client#sendInt() -> " + "Échec de l'envoi de l'entier (Interruption par une erreur IO)");
		}
	}
