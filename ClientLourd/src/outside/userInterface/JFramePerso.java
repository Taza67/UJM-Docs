...
public class JFramePerso extends JFrame implements IConfig {
	...
	public JFramePerso() {
		...
		editorContainerPanel.setBorder(BorderFactory.createEmptyBorder(DEFAULT_EDITOR_MARGE, DEFAULT_EDITOR_MARGE,
				DEFAULT_EDITOR_MARGE, DEFAULT_EDITOR_MARGE));
	...
	public void initEventManagement(boolean isOnline) {
		...
		zoomSlider.addChangeListener(new ChangeListener() {
			...
			public void stateChanged(ChangeEvent arg0) {
				...
				editorContainerPanel
						.setBorder(BorderFactory.createEmptyBorder(editorMarge, editorMarge, editorMarge, editorMarge));
		...
		// Touches
		KeyListener keyboardHandler = new KeyAdapter() {
			// Récupération du GraphicsDevice par défaut (écran principal)
			GraphicsEnvironment graphicsEnvironment = GraphicsEnvironment.getLocalGraphicsEnvironment();
			GraphicsDevice graphicsDevice = graphicsEnvironment.getDefaultScreenDevice();
			...
			public void keyPressed(KeyEvent e) {
				// Nouveau document
				if ((e.getKeyCode() == KeyEvent.VK_N) && ((e.getModifiersEx() & InputEvent.ALT_DOWN_MASK) != 0)) {
					if (isOnline) {
						String newDocument = "default";
						manager.askNewDocument(newDocument, isOnline);
					} else {
				...
				// Sauvegarde
				else if ((e.getKeyCode() == KeyEvent.VK_S) && ((e.getModifiersEx() & InputEvent.ALT_DOWN_MASK) != 0)) {
					if (isOnline) {
						manager.askSaveDocument();
					}
				}
				// Export
				else if ((e.getKeyCode() == KeyEvent.VK_E) && ((e.getModifiersEx() & InputEvent.ALT_DOWN_MASK) != 0)) {
					manager.exportDocument();
				}
				// Quitter
				else if ((e.getKeyCode() == KeyEvent.VK_Q) && ((e.getModifiersEx() & InputEvent.ALT_DOWN_MASK) != 0)) {
					self.dispose();
				}
				// Barre d'outils
				else if ((e.getKeyCode() == KeyEvent.VK_O) && ((e.getModifiersEx() & InputEvent.CTRL_DOWN_MASK) != 0)) {
					toolBar.setVisible(!isToolBarVisible);
				...
				// Recherche
				else if ((e.getKeyCode() == KeyEvent.VK_F) && ((e.getModifiersEx() & InputEvent.CTRL_DOWN_MASK) != 0)) {
				...
				// Nouvelle page
				else if ((e.getKeyCode() == KeyEvent.VK_N) && ((e.getModifiersEx() & InputEvent.CTRL_DOWN_MASK) != 0)) {
					manager.addNewPage();
				}
				// Supprimer page
				else if ((e.getKeyCode() == KeyEvent.VK_N) && ((e.getModifiersEx() & InputEvent.CTRL_DOWN_MASK) != 0)) {
	...
	private void connect(boolean isNew) {
		...
		if (pseudo.getText() == null || pseudo.getText().isEmpty() || pseudo.getText().isBlank()
			...
			System.err.println("- Appui sur le bouton " + (isNew ? "d'inscription" : "de connexion")
					+ ". Un, au moins, des champs vide");
			JOptionPane.showMessageDialog(null, "Un, au moins, des champs vide !!!", "Erreur",
		...
		if (!manager.connectApplication(pseudo.getText(), motDePasse.getText(), isNew)) {
			...
			if (isNew)
				errorMessage = "Ces identifiants existent déjà ! Appuyez sur le bouton \"Connecter\"";
			else
				errorMessage = "Pseudo ou mot de passe incorrects ! Rééssayez !";
		...
		if (!isNew) {
			...
			String choice = DialogUtilities.askChoiceFromOptions(self, "Que souhaitez-vous faire ?", "", options);
			...
			if (choice == "Nouveau document") {
				...
				// // Annulation
				if (nomFichier == null) {
					JOptionPane.showMessageDialog(self, "Vous avez décidé d'annuler. Fermeture de l'application",
							"Fermeture", JOptionPane.WARNING_MESSAGE);
				...
				while (nomFichier.isBlank() || nomFichier.isEmpty())
					nomFichier = JOptionPane.showInputDialog(self,
							"Attention ! Vous n'avez entré un nom valide ! Entrez le nom du fichier");
				...
				String[] documentsNames = new String[listDocuments.size()];
				
				System.err.println("- " + listDocuments.size());
				...
				choice = DialogUtilities.askChoiceFromOptions(self, "Charger un document", "Choisissez un document",
						documentsNames);
				
				// Indice du document dans la liste
				int i;
				for (i = 0; i < documentsNames.length; i++) {
					if (documentsNames[i] == choice)
						break;
				}
				
				// Demande au serveur
				manager.askLoadDocument(listDocuments.get(i));
			}
		}
